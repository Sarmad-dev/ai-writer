    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          await prisma.contentSession
            .update({
              where: { id: contentSession.id },
              data: { status: "FAILED" },
            })
            .catch((err) =>
              console.error("[SSE] Failed to update session status:", err)
            );
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          console.log(
            `[SSE POST] Workflow completed. Total states: ${stateCount}`
          );
          ...
          await prisma.contentSession
            ...
            .catch((err) =>
              console.error("[SSE POST] Failed to update session status:", err)
            );
